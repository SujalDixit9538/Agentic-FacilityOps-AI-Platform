...
async def energy_module_health(db: Session = Depends(get_db)):
    ...
    return success_response(message="Energy module health check", data=module_status)
...
async def seed_energy_data(
    ...
    """Trigger mock ingestion only for a facility in the canonical catalog."""
    facility_ids = {facility.facility_id for facility in FacilityCatalogService(db).list_active()}
    ...
    return success_response(
        message="Mock data pipeline executed successfully.",
...
async def get_energy_records(
    ...
    data = [EnergyRecordResponse.model_validate(record).model_dump() for record in records]
...
async def get_energy_dashboard(facility_id: str, db: Session = Depends(get_db)):
    ...
    response = success_response(
        ...
        data={
            ...
            "as_of": as_of,
            "timing_ms": {
                "query": round((time.perf_counter() - query_started) * 1000, 2)
            },
        },
